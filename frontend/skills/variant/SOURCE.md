...
- **Path in the repository:** `skills/variant`
...
**User-invoked** (`disable-model-invocation: true`), and the source states why: a design
exploration is never something to start on someone's behalf. It writes throwaway code, builds
several variants of one interface, offers a picker and then asks a question only a person can
answer.

Related to `shared/skills/prototype` (throwaway code that answers a question), but scoped to
visual variants of one interface rather than to logic or data.
